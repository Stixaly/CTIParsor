...
| 7 | CLI `main.py`, Ollama provider on the LAN station (whose HTTP endpoint the same WSL had used for minutes to generate the harness) | same, at "LLM chunk 1/1" |
...
| **F. Long-distance inference on a local model** | bind S6 to a `COMPLETION_PROVIDER`/`COMPLETION_MODEL` pair (the ADR-0033 pattern), default on when a local provider answers | **Adopted.** O(components) calls per report; on the DGX Spark station each call is free and private. Same quote bar as Stage 3d. |
...
1. [ ] Decision 1 — backend default policy + `GET /default`, frontend reset button (Qwen: `api/routes/policy.py`, `Policy.tsx`; tests: factory GET returns 27 rules, PUT of `[]` is honoured, `run_config_json` snapshot unchanged).
2. [ ] Decision 2 — label-aware Stage 3d (Qwen: `pipeline/stage3d_verify.py`; test: a `gap` claim survives with its label, a `reported` claim without quote is still removed — re-introduce the defect to prove the test bites).
3. [ ] Decision 3 — alias-aware resolution (Qwen: `pipeline/stage4_stix_mapping.py`; test: "Cozy Bear" resolves to the APT29 SDO, "snake" resolves by type — ADR-0021 fixture).
4. [ ] Decision 4 — per-node caps, separate S4/S5 budgets, stats (Qwen: `stage4_stix_mapping.py`, `stage4b_graph_completion.py`, `PolicySynthesis.tsx`).
5. [ ] Decision 5 — `COMPLETION_PROVIDER` (Qwen: `stage4c_long_distance.py`, `.env.example`, `docs/deployment.md`).
6. [ ] Decision 6 — Graph page label rendering (Qwen: `Graph.tsx`).
