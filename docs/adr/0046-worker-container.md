...
| `scripts/docker_smoke.sh --no-build --job` | **0 failures**: the sample report reached `for_review` with 33 entities and 5 relationships, a 63-object bundle; its spawn line appears in the **worker** container's log and the API container spawned nothing; 7 min 50 s wall clock including the LLM stage on an external Ollama |
