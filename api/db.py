...
_JOB_STORE_MIGRATIONS_SQLITE = [
    ...
    # STIX 2.1 SRO optional properties (spec Sec 5.1.2) - ISO 8601 strings, same
    # convention as created_at/updated_at/heartbeat_at elsewhere in this store.
    "ALTER TABLE relationships ADD COLUMN start_time TEXT",
    "ALTER TABLE relationships ADD COLUMN stop_time TEXT",
...
def _apply_postgres_ddl(pg, statements: tuple[str, ...]) -> None:
    """Run each PostgreSQL DDL statement in its own request, matching
    _apply_sqlite_migrations' "log it, don't crash the server" contract for
    the one error that is expected rather than tolerated: two processes
    racing IF-NOT-EXISTS DDL against a fresh database. One statement's race
    no longer rolls back or masks any of the others (see the module-level
    comment on _JOB_STORE_DDL_POSTGRES).
    """
